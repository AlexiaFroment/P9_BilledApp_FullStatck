...
export default class {
  constructor({ document, onNavigate, store, bills, localStorage }) {
    ...
    // Initialize counters and ids for each container
    this.counters = {}
    this.ids = {}
  ...
  handleEditTicket(e, bill, bills, containerIndex) {
    if (
      this.counters[containerIndex] === undefined ||
      this.ids[containerIndex] !== bill.id
    ) {
      this.counters[containerIndex] = 0
    }
    if (
      this.ids[containerIndex] === undefined ||
      this.ids[containerIndex] !== bill.id
    ) {
      this.ids[containerIndex] = bill.id
    }
    if (this.counters[containerIndex] % 2 === 0) {
      bills.forEach((b) => {
